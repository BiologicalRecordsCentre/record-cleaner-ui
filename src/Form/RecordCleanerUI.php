...
/********************* VERIFICATION FORM *********************/
  ...
  public function verifyCallback(array &$form, FormStateInterface $form_state) {
    ...
    // Note, I wasn't expecting to have to do this again but it is not sticking
    // in the form_state when calculated for validation.
    $columns = $this->getOutputColumns($form_state, 'validate');
    $mappings = $this->getOutputMappings($columns);
...
/********************* UTILITY FUNCTIONS *********************/
  ...
  public function submitCallback(
    ...
    $columns = $this->getOutputColumns($form_state, $action);
    $form_state->set([$output, 'columns'], $columns);
    // Determine mappings of function to column in output file.
    $mappings = $this->getOutputMappings($columns);
    $form_state->set([$output, 'mappings'], $mappings);
  ...
  public function getOutputColumns(FormStateInterface $form_state, $action) {

    /////////////////////////////////////////////
    // TODO refactor to build from input mappings.
    /////////////////////////////////////////////
  ...
  public function getOutputMappings($columns) {
    $mappings = [];
    $functions = [
